...
export default function Iscrizione() {
  ...
  async function onSubmit(dati) {
    ...
    try {
      ...
      // Registra consenso privacy per questa società
      await setDoc(
        doc(db, 'consensiPrivacy', `${utente.uid}_${societaId}_${versione}`),
        {
          uid:         utente.uid,
          societaId,
      ...
      await setDoc(
        doc(db, 'societa', societaId, 'iscrizioni', utente.uid),
        {
          ...
          preferenzeNotifiche,
          categoriaCalcolata: categorieCalcolate[0] || null,
