...
self.addEventListener('push', (event) => {
  ...
  // FCM wrappa i campi sotto "data"; gestisci entrambe le strutture
  const d     = dati.data || dati;
  const title = d.title || dati.notification?.title || 'Bacheca Scherma';
  const body  = d.body  || dati.notification?.body  || '';
  const url   = d.url   || dati.notification?.data?.url || '/scegli-societa';
